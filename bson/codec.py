...
class UnknownSerializerError(ValueError):
    def __init__(self, key, value):
        super(UnknownSerializerError, self).__init__(
            "Unable to serialize: key '%s' value: %s type: %s"
...
def encode_object(obj, traversal_stack, generator_func, on_unknown=None):
    ...
    return encode_document(
        values, traversal_stack, obj, generator_func, on_unknown
    )


def encode_object_element(
    name, value, traversal_stack, generator_func, on_unknown
):
    return (
        b"\x03"
        + encode_cstring(name)
...
def encode_value(
    name, value, buf, traversal_stack, generator_func, on_unknown=None
):
    if isinstance(value, bool):
        ...
        if value < -0x80000000 or 0x7FFFFFFFFFFFFFFF >= value > 0x7FFFFFFF:
            ...
            if value > 0xFFFFFFFFFFFFFFFF:
                raise Exception(
                    "BSON format supports only int value < %s"
                    % 0xFFFFFFFFFFFFFFFF
                )
...
def encode_document(
    ...
    for name in key_iter:
        ...
        encode_value(
            name, value, buf, traversal_stack, generator_func, on_unknown
        )
        traversal_stack.pop()
    e_list = buf.getvalue()
    e_list_length = len(e_list)
    return struct.pack(
        "<i%dsb" % (e_list_length,), e_list_length + 4 + 1, e_list, 0
    )
...
def encode_array(
    ...
    for i in range(0, len(array)):
        ...
        encode_value(
            str(i), value, buf, traversal_stack, generator_func, on_unknown
        )
        traversal_stack.pop()
    e_list = buf.getvalue()
    e_list_length = len(e_list)
...
def encode_binary_element(name, value, binary_subtype=0):
    return (
        b"\x05"
        + encode_cstring(name)
        + encode_binary(value, binary_subtype=binary_subtype)
    )
